            ...
            <tr>
              ...
              <td width="110">
                <div>
                  ...
                  @if($costo->pbix != null)
                    <a href="{{ Storage::url($costo->pbix) }}" class="btn btn-primary btn-sm" title="Descargar Archivo" data-position="top" data-delay="50" data-tooltip="Descargar Archivo" download>Descargar</a>
                  @else
                ...
                <div aling="center">
                  @can('editar-costo')
                    <button class="btn btn-info btn-sm" data-id="{{$costo->id}}" data-titulo="{{$costo->titulo}}" data-fecha="{{$costo->fecha}}" data-obs="{{$costo->obs}}" data-pbix="{{$costo->pbix}}" data-frecuencia="{{$costo->frecuencia}}" data-toggle="modal" data-target="#editar"> Editar</button>
                  @endcan
                  @can('eliminar-costo')
                    <a href="{{url('destroy_costo', $costo->id)}}" class="btn btn-danger btn-sm" title="Borrar" onclick="return confirm ('Está seguro que desea eliminar el archivo?')"data-position="top" data-delay="50" data-tooltip="Borrar">X</a>
                  @endcan
                </div>
              </td>
            </tr>
