...
class VisitaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($tarjeta && $tarjeta->libre == 1) {
            
            DB::table('tarjetas')->where('id_tar', $request['tarjeta'])->update(['libre' => 0]);
            ...
            Session::flash('message', 'La tarjeta seleccionada no está disponible o no existe');
    ...
    {
        $tarjetaId = $request->input('id');
        $tarjeta = Tarjeta::find($tarjetaId);
        
        if ($tarjeta && $tarjeta->libre==0) {//si la tarjeta esta ocupada entra 
            
            $visita = Visita::where('tarjeta', $tarjetaId)->where('activa', 1)->first();
            ...
            Session::flash('message', 'La tarjeta seleccionada no está activa o no existe');
            Session::flash('alert-class', 'alert-danger');
        }
